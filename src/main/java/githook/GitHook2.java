...
public class GitHook2 {
    private File toCommitFile;
    private List<String> lines;

    public GitHook2(File toCommitFile) throws IOException {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Ref ref = getBranchRef(dotGit, "dev3_QA");
            ...
            GitStatusResponse gitStatus = wt.getStatus();

            System.out.println("untractedFileSize : " + gitStatus.getUntrackedFilesSize());
            System.out.println("modifiedFileSize : " + gitStatus.getModifiedFilesToCommitSize());
            System.out.println("newFileSize : " + gitStatus.getNewFilesToCommitSize());
            
            FileUtils.copyFile(new File("test1.txt"), new File(workingTreePath, "test1.txt"));
            
            gitStatus = wt.getStatus();
            
            System.out.println("untractedFileSize : " + gitStatus.getUntrackedFilesSize());
            System.out.println("modifiedFileSize : " + gitStatus.getModifiedFilesToCommitSize());
            System.out.println("newFileSize : " + gitStatus.getNewFilesToCommitSize());

            //List<File> modifiedOrNewFiles = scanModifiedOrNewFiles(gitStatus);
     ...
     * @throws IOException
     */
    private static Ref getBranchRef(DotGit dotGit, String string) throws IOException, JavaGitException {
        while (dotGit.getBranches().hasNext()) {
            Ref ref = dotGit.getBranches().next();
            if (string.equals(ref.getName())) {
                return ref;
            }
        }
        throw new IllegalArgumentException(string + " is not found.");
    }
